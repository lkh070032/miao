  ...
  return Math.round(number * result) / result;
   ...
   function parserJSON(JSON) {
        ...
        while (i < JSON.length) {
            if (JSON[i] === '"') {
                ...
                if (JSON[i] === ':') {
                    ...
                    throw new SyntaxError('Expected ":" at index ' + i);
                }
            } else if (JSON[i] === ',') {
                i++;
            } else if (JSON[i] === '}') {
                ...
                throw new SyntaxError('Unexpected character in object at index ' + i);
            }
        }
        throw new SyntaxError('Unexpected end of input in object');
    }
    function parseArray() {
        var result = [];
        i++; // skip '['
        ...
        throw new SyntaxError('Unexpected end of input in array');
    }
    function parseString() {
        ...
        if (JSON[i] !== '"') {
            throw new SyntaxError('Unterminated string at index ' + (i - 1));
        }
        i++; // skip closing '"'
        return result;
    }
    function parseNumber() {
        var result = '';
        while (i < JSON.length && (JSON[i] >= '0' && JSON[i] <= '9' || JSON[i] === '-' || JSON[i] === '.' || JSON[i] === 'e' || JSON[i] === 'E' || JSON[i] === '+')) {
            result += JSON[i];
            i++;
        }
